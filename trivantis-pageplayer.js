...
jsPagePlayer.prototype.loadPage = function(pageName, cb)
{
	...
	if(this.bkAudioObj == n)
		jsOpaUtils.createBkAudioPlayer(this);
	
	
	
	if(is.isMobile.any())
	{
		if(this.inheritedDiv == n)
			this.inheritedDiv = jsOpaUtils.createInheritedDIV();
		
		jsOpaUtils.createAutoStartMedia(this);
	}
...
jsPagePlayer.prototype.cleanUpAutoStartObj = function ()
{
	var div = n;
	if(this.autoAudioObj)
	{
		div = document.getElementsByClassName("pgAutoAudio");
		if(div.length)
		{
			div = div[0];
			//This is one that belongs to the page
			if(div.id != "pgAutoAudio")
			{
				var id = div.id+'TMP';
				var obj = eval(div.id+'Object');
				if(obj)
					obj.actionStop();
				
				//LD-4624 - unset player
				obj.rebuildMediaPlayer(n,n,n);
				
				var realDiv = document.getElementById(id);
				if(realDiv)
				{
					realDiv.id = div.id;
					realDiv.style.visibility = div.style.visibility;
					div.style.visibility = 'hidden';
					div.id = "pgAutoAudio";
					this.autoAudioObj.actionChangeContents('media/blank.mp4');
				}
			}
		}
		//LD-4601
		if(!this.bFirstClickGrabbed)
		{
			this.autoAudioObj.actionPlay();
			this.autoAudioObj.actionStop();
		}
	}
	
	div = n;
	if(this.autoVideoObj)
	{
		div = document.getElementsByClassName("pgAutoVideo");
		if(div.length)
		{
			div = div[0];
			//This is one that belongs to the page
			if(div.id != "pgAutoVideo")
			{
				var id = div.id+'TMP';
				var obj = eval(div.id+'Object');
				if(obj)
				...
				if(realDiv)
				{
					...
					div.style.visibility = 'hidden';
					div.id = "pgAutoVideo";
					this.autoVideoObj.actionChangeContents('media/blank.mp4');
				}
				this.autoVideoObj.mediaPlayer.controls.removeClass("mejs-roll");
			}
		}
		//LD-4601
		if(!this.bFirstClickGrabbed)
		{
			this.autoVideoObj.actionPlay();
			this.autoVideoObj.actionStop();
			this.autoVideoObj.div.style.display ='';
		}
	}
	
	//LD-4601
	if(this.autoAudioObj && this.autoVideoObj)
	{
		if(!this.bFirstClickGrabbed)
			this.bFirstClickGrabbed = true;
	}
};
...
jsPagePlayer.prototype.postPageShow = function(thePage)
{
	if(thePage.iframe.contentWindow.bPageLoaded)
	{
		...
		jsOpaUtils.transferBkAudio(thePage);
		jsOpaUtils.playAutoStartMedia(thePage);
		//this is to set the page status after pageshow actions have been fired.
		thePage.iframe.contentWindow.postPageShowAction(); 
		window.scrollTo(0,0);
